/*
| This is an autogenerated file.
|
| DO NOT EDIT!
*/


/*
| Export
*/
var
	Forms =
		Forms || { };


/*
| Imports
*/
var
	JoobjProto,
	Jools;


/*
| Capsule
*/
(function( ) {
'use strict';


var
	_tag =
		638172488;


/*
| Constructor.
*/
var NonExistingSpace =
Forms.NonExistingSpace =
	function(
		tag,
		inherit,        // inheritance
		ranks,          // twig order, set upon change
		twig,           // twig, set upon change
		v_hover,        // the widget hovered upon
		v_mark,         // the users mark
		v_nonSpaceTag,  // the tag part of the non-existing-space
		v_nonSpaceUser, // the user part of the non-existing-space
		v_path,         // the path of the form
		v_view          // the current view
	)
{

/**/if( CHECK )
/**/{
/**/	if( tag !== _tag )
/**/	{
/**/		throw new Error(
/**/			'tag mismatch'
/**/		);
/**/	}
/**/}

	this.hover =
		v_hover;

	this.mark =
		v_mark;

	if( v_nonSpaceTag !== undefined )
	{
		this.nonSpaceTag =
			v_nonSpaceTag;
	}

	if( v_nonSpaceUser !== undefined )
	{
		this.nonSpaceUser =
			v_nonSpaceUser;
	}

	if( v_path !== undefined )
	{
		this.path =
			v_path;
	}

	if( v_view !== undefined )
	{
		this.view =
			v_view;
	}

	this.twig =
		twig;

	this.ranks =
		ranks;

	this._init(
		inherit
	);

	Jools.immute( this );
Jools.immute( ranks );
Jools.immute( twig );
};


Jools.subclass(
	NonExistingSpace,
	Forms.Form
);


/*
| Creates a new NonExistingSpace object.
*/
NonExistingSpace.create =
NonExistingSpace.prototype.create =
	function(
		// free strings
	)
{
	var
		inherit,
		key,
		ranks,
		twig,
		twigdup,
		v_hover,
		v_mark,
		v_nonSpaceTag,
		v_nonSpaceUser,
		v_path,
		v_spaceTag,
		v_spaceUser,
		v_username,
		v_view;

	if( this !== NonExistingSpace )
	{
		inherit =
			this;

		twig =
			inherit.twig;

		ranks =
			inherit.ranks;

		twigdup =
			false;

		v_hover =
			this.hover;

		v_mark =
			this.mark;

		v_nonSpaceTag =
			this.nonSpaceTag;

		v_nonSpaceUser =
			this.nonSpaceUser;

		v_path =
			this.path;

		v_view =
			this.view;
	}
	else
	{
		twig =
			{ };

		ranks =
			[ ];

		twigdup =
			true;
	}

	for(
		var a = 0, aZ = arguments.length;
		a < aZ;
		a += 2
	)
	{
		var
			arg =
				arguments[ a + 1 ];

		switch( arguments[ a ] )
		{
			case 'hover' :

				if( arg !== undefined )
				{
					v_hover =
						arg;
				}

				break;

			case 'mark' :

				if( arg !== undefined )
				{
					v_mark =
						arg;
				}

				break;

			case 'nonSpaceTag' :

				if( arg !== undefined )
				{
					v_nonSpaceTag =
						arg;
				}

				break;

			case 'nonSpaceUser' :

				if( arg !== undefined )
				{
					v_nonSpaceUser =
						arg;
				}

				break;

			case 'path' :

				if( arg !== undefined )
				{
					v_path =
						arg;
				}

				break;

			case 'spaceTag' :

				if( arg !== undefined )
				{
					v_spaceTag =
						arg;
				}

				break;

			case 'spaceUser' :

				if( arg !== undefined )
				{
					v_spaceUser =
						arg;
				}

				break;

			case 'username' :

				if( arg !== undefined )
				{
					v_username =
						arg;
				}

				break;

			case 'view' :

				if( arg !== undefined )
				{
					v_view =
						arg;
				}

				break;

			case 'twig:add' :

				if( !twigdup )
				{
					twig =
						Jools.copy( twig );

					ranks =
						ranks.slice( );

					twigdup =
						true;
				}

				key =
					arg;

				arg =
					arguments[ ++a + 1 ];

				if( twig[ key ] !== undefined )
				{
					throw new Error(
						'key "' + key + '" already in use'
					);
				}

				twig[ key ] =
					arg;

				ranks.push( key );

				break;

			case 'twig:set' :

				if( !twigdup )
				{
					twig =
						Jools.copy( twig );

					ranks =
						ranks.slice( );

					twigdup =
						true;
				}

				key =
					arg;

				arg =
					arguments[ ++a + 1 ];

				if( twig[ key ] === undefined )
				{
					throw new Error(
						'key "' + key + '" not in use'
					);
				}

				twig[ key ] =
					arg;

				break;

			default :

/**/			if( CHECK )
/**/			{
/**/				throw new Error(
/**/					'invalid argument: ' + arguments[ a ]
/**/				);
/**/			}
		}
	}

	if( v_hover === undefined )
	{
		v_hover =
			null;
	}

	if( v_mark === undefined )
	{
		v_mark =
			null;
	}

	if( v_nonSpaceTag === undefined )
	{
		v_nonSpaceTag =
			undefined;
	}

	if( v_nonSpaceUser === undefined )
	{
		v_nonSpaceUser =
			undefined;
	}

	if( v_path === undefined )
	{
		v_path =
			undefined;
	}

	if( v_spaceTag === undefined )
	{
		v_spaceTag =
			undefined;
	}

	if( v_spaceUser === undefined )
	{
		v_spaceUser =
			undefined;
	}

	if( v_username === undefined )
	{
		v_username =
			null;
	}

	if( v_view === undefined )
	{
		v_view =
			undefined;
	}

/**/if( CHECK )
/**/{
/**/
/**/	if( v_hover === undefined )
/**/	{
/**/		throw new Error(
/**/			'undefined attribute hover'
/**/		);
/**/	}
/**/
/**/	if( v_hover !== null )
/**/	{
/**/		if( v_hover.reflect !== 'Path' )
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_mark === undefined )
/**/	{
/**/		throw new Error(
/**/			'undefined attribute mark'
/**/		);
/**/	}
/**/
/**/	if( v_nonSpaceTag === null )
/**/	{
/**/		throw new Error(
/**/			'nonSpaceTag must not be null'
/**/		);
/**/	}
/**/
/**/	if( v_nonSpaceTag !== undefined )
/**/	{
/**/		if(
/**/			typeof( v_nonSpaceTag ) !== 'string'
/**/			&&
/**/			!( v_nonSpaceTag instanceof String )
/**/		)
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_nonSpaceUser === null )
/**/	{
/**/		throw new Error(
/**/			'nonSpaceUser must not be null'
/**/		);
/**/	}
/**/
/**/	if( v_nonSpaceUser !== undefined )
/**/	{
/**/		if(
/**/			typeof( v_nonSpaceUser ) !== 'string'
/**/			&&
/**/			!( v_nonSpaceUser instanceof String )
/**/		)
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_path === null )
/**/	{
/**/		throw new Error(
/**/			'path must not be null'
/**/		);
/**/	}
/**/
/**/	if( v_path !== undefined )
/**/	{
/**/		if( v_path.reflect !== 'Path' )
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_spaceTag === null )
/**/	{
/**/		throw new Error(
/**/			'spaceTag must not be null'
/**/		);
/**/	}
/**/
/**/	if( v_spaceTag !== undefined )
/**/	{
/**/		if(
/**/			typeof( v_spaceTag ) !== 'string'
/**/			&&
/**/			!( v_spaceTag instanceof String )
/**/		)
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_spaceUser === null )
/**/	{
/**/		throw new Error(
/**/			'spaceUser must not be null'
/**/		);
/**/	}
/**/
/**/	if( v_spaceUser !== undefined )
/**/	{
/**/		if(
/**/			typeof( v_spaceUser ) !== 'string'
/**/			&&
/**/			!( v_spaceUser instanceof String )
/**/		)
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_username === undefined )
/**/	{
/**/		throw new Error(
/**/			'undefined attribute username'
/**/		);
/**/	}
/**/
/**/	if( v_username !== null )
/**/	{
/**/		if(
/**/			typeof( v_username ) !== 'string'
/**/			&&
/**/			!( v_username instanceof String )
/**/		)
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/
/**/	if( v_view === null )
/**/	{
/**/		throw new Error(
/**/			'view must not be null'
/**/		);
/**/	}
/**/
/**/	if( v_view !== undefined )
/**/	{
/**/		if( v_view.reflect !== 'View' )
/**/		{
/**/			throw new Error(
/**/				'type mismatch'
/**/			);
/**/		}
/**/	}
/**/}
	v_mark =
		Forms.Form.concernsMark(
			v_mark,
			v_path
		);

	v_view =
		v_view !== undefined ?
			v_view.sizeOnly:
			null;

	if(
		inherit
		&&
		!twigdup
		&&
		(
			v_hover === inherit.hover
			||
			(
				v_hover
				&&
				v_hover.equals( inherit.hover )
			)
		)
		&&
		v_mark === inherit.mark
		&&
		v_nonSpaceTag === inherit.nonSpaceTag
		&&
		v_nonSpaceUser === inherit.nonSpaceUser
		&&
		(
			v_path === inherit.path
			||
			(
				v_path
				&&
				v_path.equals( inherit.path )
			)
		)
		&&
		v_spaceTag === null
		&&
		v_spaceUser === null
		&&
		v_username === null
		&&
		(
			v_view === inherit.view
			||
			(
				v_view
				&&
				v_view.equals( inherit.view )
			)
		)
	)
	{
		return inherit;
	}

	return (
		new NonExistingSpace(
			_tag,
			inherit,
			ranks,
			twig,
			v_hover,
			v_mark,
			v_nonSpaceTag,
			v_nonSpaceUser,
			v_path,
			v_view
		)
	);
};


/*
| Reflection.
*/
NonExistingSpace.prototype.reflect =
	'NonExistingSpace';


/*
| Sets values by path.
*/
NonExistingSpace.prototype.setPath =
	JoobjProto.setPath;


/*
| Gets values by path.
*/
NonExistingSpace.prototype.getPath =
	JoobjProto.getPath;


/*
| .
*/
NonExistingSpace.prototype.atRank =
	JoobjProto.atRank;


/*
| Checks for equal objects.
*/
NonExistingSpace.prototype.equals =
	function(
		obj
	)
{
	if( this === obj )
	{
		return true;
	}

	if( !obj )
	{
		return false;
	}

	return (
		this.twig === obj.twig
		&&
		(
			this.hover === obj.hover ||
			(
				this.hover !== null
				&&
				this.hover.equals( obj.hover )
			)
		)
		&&
		this.mark === obj.mark
		&&
		this.nonSpaceTag === obj.nonSpaceTag
		&&
		this.nonSpaceUser === obj.nonSpaceUser
		&&
		this.path.equals( obj.path )
		&&
		this.view.equals( obj.view )
	);
};


} )( );
